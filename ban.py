...
def useragent_facebook():
    ...
    ubuntu = random.choice(["X11; Ubuntu; Linux x86_64", "X11; Ubuntu; Linux i686"])
    angka = random.randint(100, 999)
    linucx = random.choice(["Ubuntu", "Linux"])
    arm = random.choice(["64", "32"])
    windows = random.choice(["Win64; x64", "Win32; x86"])
    tahun = random.randint(2019, 2023)
...
def get_code_temp_plus(email):
    mail = requests.Session()
    headers = {
        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.1 Mobile/15E148 Safari/604.1/1220.1 Darwin/20.3.0',
        'Content-Type': 'application/json',
        'Connection-type': 'wifi'
    }
    
    # বাকি কোড এখানে যোগ করুন
    # ...
...
def main() -> None:
    ...
    for make in range(int(num_accounts)):
        ...
        m_ts = re.search(r'name="m_ts" value="(.*?)"', str(mts)).group(1) if re.search(r'name="m_ts" value="(.*?)"', str(mts)) else str(int(time.time()))
        ...
        payload = {
            ...
            'reg_instance': str(formula["reg_instance"]) if "reg_instance" in formula else "",
            'helper': "",
            'reg_impression_id': str(formula["reg_impression_id"]) if "reg_impression_id" in formula else f"imp_{random.randint(100000,999999)}",
            'ns': "1",
            'zero_header_af_client': "",
            'app_id': "103",
            'logger_id': str(formula["logger_id"]) if "logger_id" in formula else f"log_{random.randint(100000,999999)}",
            ...
            'submit': "Sign Up",
            'fb_dtsg': ":0:0",
            'jazoest': str(formula["jazoest"]) if "jazoest" in formula else "1234",
            'lsd': str(formula["lsd"]) if "lsd" in formula else "AVpB_V8c",
        ...
        'priority': "u=1, i"}
        response = ses.post('https://facebook.com/reg', data=payload, headers=header1)
